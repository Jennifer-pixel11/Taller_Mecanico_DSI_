...
// 4️⃣ Obtener servicios asociados a los vehículos del cliente
$stmtServicios = $conn->prepare("
    SELECT s.id, s.costo, s.fecha, s.descripcion,
    ...
    WHERE c.correo = ?
");
$stmtServicios->bind_param("s", $cliente['correo']);
    ...
    <div class="col-md-10">
      <div class="card shadow-lg border-0">
        ...
        <div class="card-body">
              ...
              <tbody>
                <?php while ($servicio = $servicios->fetch_assoc()): ?>
                 <tr>
                    ...
                    <td>$<?= htmlspecialchars(number_format($servicio['costo'], 2)); ?></td>
                    <td><?= htmlspecialchars($servicio['marca']) . " " . htmlspecialchars($servicio['modelo']) . " (" . htmlspecialchars($servicio['placa']) . ")"; ?></td>
                    <td>
                        ...
                        <input type="hidden" name="item_name" value="<?= $servicio['marca']  ?>" />
          ...
          <?php else: ?>
            <p class="text-center">No hay servicios asociados a tus vehículos.</p>
          <?php endif; ?>
        </div>
      </div>
    </div>
